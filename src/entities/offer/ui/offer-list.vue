...
<template>
  <div :class="cn(
    ...
    " v-for="item in offersItems" :key="item.itemId" @click="handleItemClick(item)">
    <div class="flex w-full flex-col">
      <div class="flex w-full flex-row items-center justify-between">
        <h3 class="text-[14px] font-medium text-[#101828]">{{ item.title }}</h3>
      </div>
      <h3 v-if="item.price" :key="item.price.formatted" class="text-[16px] font-bold text-[#101828]">
    ...
    <div class="flex w-full flex-row justify-between">
      ...
      <p class="text-[14px] font-normal text-[#667085]" @click.stop="handleVendorClick(item.vendor_title)">
        {{ item.vendor_title ? item.vendor_title : 'Не указано' }}
      </p>
