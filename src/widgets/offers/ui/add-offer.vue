...
<template>
  <div class="flex h-screen w-full flex-col justify-between bg-white lg:max-w-[355px]">
    <div
      ...
      <div class="group flex items-center gap-x-2" @click="handleClose">
        ...
        <p class="text-center text-[17px] text-primary group-hover:text-primary/70">Закрыть</p>
      </div>
    </div>
    <form @submit.prevent="onSubmit" class="flex h-[calc(100%-64px)] flex-col justify-between border-l border-r border-[#D0D4DB] bg-white">
      <div class="flex flex-col gap-y-4 p-4">
        <p class="mb-4 text-[18px] font-semibold text-[#101828]">Добавление предложения</p>
        <FormInput name="name" label="Наименование" placeholder="Наименование" />
        <FormInput number name="amount" label="Количество" placeholder="Количество" />
      ...
      <div class="w-full border-t border-[#CCD0D9] bg-[#F9FAFB] p-4 md:min-w-[305px]">
        <Button class="w-full text-[17px] font-semibold" type="submit">Добавить</Button>
      </div>
    </form>
  </div>
</template>
