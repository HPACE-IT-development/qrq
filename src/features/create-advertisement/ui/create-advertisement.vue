...
const { data: destinationsArray } = useUnit(getDestinationsQuery);

const { data: categories } = useUnit(getCategories);
const { data: brands } = useUnit(getBrands);

useGate(nomenclaturesGate);
const loading = useStore(getNomenclaturesQuery.$pending);
...
<template>
  <div
    ...
    <div class="flex w-full items-center justify-start gap-x-2 border-b border-[#D0D4DB] px-4 py-2">
      ...
      <Button v-if="formMode === 'form'" @click="handleClose" variant="ghost" size="icon" class="ml-auto">
    ...
    <ScrollArea
      ...
      <template v-if="formMode === 'selectType'">
        ...
        <div class="mt-6 flex w-full gap-x-4 px-5">
          <Button class="w-full rounded-[10px] text-[16px]" @click="handleSelectedType('buy')">
            Купить
          </Button>
          <Button class="w-full rounded-[10px] text-[16px]" @click="handleSelectedType('sell')">
            Продать
          </Button>
        </div>
      </template>
      <template v-else>
        <form @submit="onSubmit" class="my-5 flex w-full flex-col gap-y-4 px-5">
          <suggestion-input v-model='name' :options='names' label='Наименование' :loading='loading'
            @find='value => handleFindNomenclatures(value)'
            @select="(selectedName: any) => selectData(selectedName, 'name')" />
          <suggestion-input v-model='article' :options='articles' label='Артикул' :loading='loading'
            @find='value => handleFindNomenclatures(value)'
            ...
            <FormItem>
              ...
              <FormControl>
                <Input
                  ...
                  type="number" placeholder="Количество" v-bind="componentField" />
              </FormControl>
              <FormMessage />
            </FormItem>
          </FormField>
          <FormField v-slot="{ componentField }" name="price" v-if="advertisementType === 'sell'">
            <FormItem>
              <FormLabel class='pb-2 text-[14px] font-semibold text-[#101828]'>Цена</FormLabel>
              <FormControl>
                <Input
                  class="h-fit rounded-[8px] border border-[#D0D4DB] px-4 py-2 text-[16px] placeholder:text-[#858FA3]"
                  type="number" placeholder="Цена" v-bind="componentField" />
              </FormControl>
              <FormMessage />
            </FormItem>
          </FormField>
          <FormField v-if="advertisementType === 'sell'" v-slot="{ componentField }" name="delivery_time">
            <FormItem>
              <FormLabel class='pb-2 text-[14px] font-semibold text-[#101828]'>Срок до клиента</FormLabel>
              <FormControl>
                <Input
                  class="h-fit rounded-[8px] border border-[#D0D4DB] px-4 py-2 text-[16px] placeholder:text-[#858FA3]"
                  type="number" placeholder="Срок до клиента" v-bind="componentField" />
              </FormControl>
              <FormMessage />
            </FormItem>
          </FormField>
          <div class="relative inline-block text-left">
            <Listbox :model-value="selectedDestinations" multiple @update:model-value='changeDestinations'>
              ...
              <ListboxButton
                ...
                <p :class="cn(
                  ...
                  ">
                  {{ selectedDestinations?.length > 0 ? selectedDestinations?.map((destination: any) =>
                    destination.name).join(', ') : 'Назначение'
                  }}
                </p>
              ...
              <transition leave-active-class="transition ease-in duration-100" leave-from-class="opacity-100"
                ...
                <ListboxOptions
                  ...
                  <ListboxOption v-if="destinationsArray" v-for="item in destinationsArray?.data" :value="item"
                    v-slot='{ selected }'>
                    <li :class="cn(
            ...
            <Listbox v-model="category">
              ...
              <transition leave-active-class="transition ease-in duration-100" leave-from-class="opacity-100"
                ...
                <ListboxOptions
                  ...
                  <ListboxOption v-if="categories?.data" v-for="item in categories?.data" :value="item.id">
                    <li :class="cn(
                      'mx-1 my-1 cursor-pointer select-none rounded py-2 pl-3 pr-9 text-gray-900 hover:bg-opacity-90',
            ...
            <Listbox v-model="brand">
              ...
              <transition leave-active-class="transition ease-in duration-100" leave-from-class="opacity-100"
                ...
                <ListboxOptions
                  ...
                  <ListboxOption v-if="brands?.data" v-for="item in brands?.data" :value="item.id">
                    <li :class="cn(
                      'mx-1 my-1 cursor-pointer select-none rounded py-2 pl-3 pr-9 text-gray-900 hover:bg-opacity-90',
    ...
    <div v-if="advertisementType" class="mt-auto flex w-full items-center justify-center border-t border-[#D0D4DB] p-4">
    ...
    <TransitionRoot :show="popoverOpened" as="div">
      ...
      <div class='absolute left-0 bottom-0 z-3'>
        <TransitionChild appear as='div' enter="transition-all duration-300" enter-from="opacity-0"
          enter-to="opacity-100" leave="transition-all duration-350" leave-from="opacity-100" leave-to="opacity-0">
