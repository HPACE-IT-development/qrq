...
<template>
  <div v-if="isProductCardOpen"
    class="flex h-screen w-full min-w-[360px] flex-col justify-between border-l border-[#D0D4DB] bg-white lg:w-[360px]">
    <div class="flex flex-col overflow-hidden">
      <div>
        <div class="group flex cursor-pointer items-center gap-x-2 border-b border-[#D0D4DB] px-2 py-4"
          @click="closeProduct">
      ...
      <ScrollArea v-if="step < 4 && !isPurchased" class="w-full">
        <div class='overflow-y-auto custom-scrollbar'>
          <Carousel @init-api="setApi" class="relative ml-[1px]" v-if="!!props.productItem.photo">
            <CarouselContent>
          ...
          <div v-if="props.productItem" class="px-4 mb-9 mt-4 custom-scrollbar">
            ...
            <div class="flex w-full flex-col gap-y-2 font-normal text-[#858FA3]">
              ...
              <div v-if="props.productItem.quantity" class="flex flex-row justify-between">
                <p>Количество</p>
                <p class="font-normal text-[#101828] [&>p]:text-end">
                  {{ props.productItem.quantity.formatted }}
                </p>
              </div>
              <div v-if="props.productItem.vendor_title" class="flex flex-row justify-between">
                <p>Поставщик</p>
                <p class="font-normal text-[#101828] [&>p]:text-end">
                  {{ props.productItem.vendor_title }}
                </p>
              </div>
              <div v-if="props.productItem.warehouse" class="flex flex-row justify-between">
                <p>Местоположение</p>
                <p class="font-normal text-[#101828] [&>p]:text-end">
                  {{ props.productItem.warehouse }}
                </p>
              </div>
              <div v-if="props.productItem.parsedDelivery" class="flex flex-row justify-between">
                <p>Срок поставки</p>
                <p class="font-normal text-[#101828] [&>p]:text-end">
              ...
              <template v-if="showDetails">
                <div v-for="field in fields" :key="field.fieldId" class='flex flex-col gap-4'>
                  <div class='flex flex-col gap-2'>
                    ...
                    <FormField v-if="field.typeName === 'SelectField'" v-slot="{ componentField }"
                      ...
                      <FormItem>
                        <Select v-bind="componentField" v-model="fieldsObject[field.fieldName]">
                          ...
                          <SelectContent>
                            <SelectItem v-for="(option, index) in field.options" :key="index" :value="option.value">
                              {{ option.text }}
                            </SelectItem>
                          </SelectContent>
                        </Select>
                      ...
                      <FormItem>
                        <FormControl>
                          <Input
                            ...
                            type="text" :placeholder="field.title" v-bind="componentField"
