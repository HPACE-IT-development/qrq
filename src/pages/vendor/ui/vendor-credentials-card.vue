...
import { ref } from "vue";
import { createAccountQuery, getAccountsQuery, useVendorCredentialsForm } from "@/pages/vendor/model/vendor-credentials-model";
...
const createAccount = async (vendorModel: any) => {
...
<template>
  <div class="flex w-full flex-col border-l bg-white lg:max-w-[355px]">
    <div class="w-full items-center border-b border-[#D0D4DB] px-0 py-3">
      <Button class="flex gap-x-2" variant="ghost" @click="closeCard()">
    ...
    <div v-if='!showAddedMessage' class="h-[calc(100vh-177px)] w-full bg-[#F9FAFB]">
      <form class="mt-4 flex w-full flex-col gap-y-4 px-5" @submit.prevent="onSubmit">
          ...
          <FormItem>
            ...
            <FormControl>
              <Select v-bind="componentField">
                ...
                <SelectContent>
                  <SelectGroup>
                    <SelectItem v-for="item of selectedVendor.qwep_vendors" :value="item.title">
                      {{ item.title }}
                    </SelectItem>
                  </SelectGroup>
                </SelectContent>
          ...
          <FormItem>
            ...
            <FormControl>
              <Input
                ...
                type="text" placeholder="Логин" v-bind="componentField" />
            </FormControl>
            <FormMessage />
          </FormItem>
        </FormField>
    ...
    <div v-if='!showAddedMessage'
      ...
      <Button @click.prevent="onSubmit" variant='tertiary' class="w-full text-base font-semibold mt-4"
        type="button">Добавить в Интересы</Button>
    </div>
  </div>
</template>